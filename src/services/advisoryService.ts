...
export async function generateVillageAdvisory(
  ...
  // 1. Fetch real active alerts from Firestore/service
  let alerts = await getAshaAlertsAsync();
  if (!alerts || alerts.length === 0) {
    alerts = getAshaAlerts();
  }

  // Aggregate symptoms from real session escalations
  ...
  // 2. Read RAG Corpus guidelines
  let corpusText = '';
  try {
    const imciPath = path.join(process.cwd(), 'data', 'triage_corpus', 'who_imci_guidelines.txt');
    const icmrPath = path.join(process.cwd(), 'data', 'triage_corpus', 'icmr_nhm_protocols.txt');
    if (fs.existsSync(imciPath)) corpusText += fs.readFileSync(imciPath, 'utf8') + '\n\n';
    if (fs.existsSync(icmrPath)) corpusText += fs.readFileSync(icmrPath, 'utf8') + '\n\n';
  } catch (err) {
...
ICMR/NHM Protocols Section 2: Adult Emergency Red Flags (chest pain, stroke FAST signs, severe dyspnea, maternal hemorrhage).
  ...
  if (!apiKey) {
    // Return structured default if key missing
